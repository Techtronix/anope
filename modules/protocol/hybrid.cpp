...
class HybridProto : public IRCDProto
{
  ...
  public:
	...
	{
		UplinkSocket::Message(Me) << "DLINE * " << (x->expires ? x->expires - Anope::CurTime : 0) << " " << x->GetHost() << " :" << x->GetReason();
	}
	...
	void SendAkill(User *u, XLine *x) anope_override
	{
		...
		UplinkSocket::Message(Me) << "KLINE * " << (x->expires ? x->expires - Anope::CurTime : 0) << " " << x->GetUser() << " " << x->GetHost() << " :" << x->GetReason();
	}
